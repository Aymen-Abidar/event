...
export async function createBookingAction(formData: FormData) {
  ...
  const eventDate = new Date(`${input.event_date}T00:00:00`);
  const deliveryDate = new Date(`${input.delivery_date}T00:00:00`);
  const returnDate = new Date(`${input.return_date}T00:00:00`);
  ...
  redirect("/bookings?success=created");
}
